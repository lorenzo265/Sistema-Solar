...
Camada que conhece o estado do jogo. Usa renderer.draw_text como
unica primitiva. renderer.py nao precisa saber nada sobre HUD,
controles, missoes ou velocidade do tempo.
"""
...
def _anchor_of(bodies):
    """O corpo mais massivo serve de referencia para telemetria orbital."""
...
def draw(bodies, height, paused, time_scale_index, show_forces, mission=None):
    """
    Renderiza painel de status, lista de corpos, controles e missao ativa.
    """
    y = height - 20
    ...
    for i, body in enumerate(bodies[:HUD_MAX_PLANETS]):
        ...
        # Telemetria orbital extra (Feature 2): so para corpos que orbitam
        # algum outro corpo mais massivo. Mostra distancia e se a orbita e ligada.
